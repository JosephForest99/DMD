#' Simulador de aclareos grafico con el modelo de Quinonez Barraza y Ramirez Maldonado (2019)
#'
#' La funcion permite simular aclareos de forma grafica utilizando el indice de densidad de Quinonez-Barraza y Ramirez-Maldonado (2019)
#'
#' @param x Objeto creado con la funcion \code{link{sim_aclareos_qr}}
#'
#' @return Regresa una grafica con los resultados de la simulacion tabular de la funcion \code{link{sim_aclareos_qr}}
#'
#' @seealso \code{\link{sim_aclareos_qr}}
#'
#' @export
#'
#' @import ggplot2

plot_sim_aclareos_qr <- function(x){

  if (names(x[1]) != "simulacion_qr") {

    stop("El objeto que debe ingresar es el obtenido con la funcion 'sim_aclareos_qr'")
    ...
    df2$B1 <- B1; df2$N <- df2$IDR * exp(df2$B1*(df2$DCM-DCMR))

#' Simulador de aclareos tabular con el modelo de Quinonez-Barraza y Ramirez-Maldonado (2019)
#'
#' La funcion permite simular aclareos utilizando el indice de densidad de Quinonez-Barraza y Ramirez-Maldonado (2019)
#'
...
#' @export sim_aclareos_qr

sim_aclareos_qr <- function(IDR_max, N, DCM, DCMR = 25, B1 = -0.0532,
  ...
  if (IDR_max <= 0 | N <= 0 | DCM <= 0 | DCMR <= 0) {
    ...
    if (IDRrel > lim_autoclareo) {
      ...
      for (i in 1:nrows) {
        ...
        df[1+i,3] = ifelse(i %% 2 == 1, IDRinf*exp(B1*(df[i,2]-DCMR)), df[i,3])

        # 2° Columna Diámetro Cuadrático Medio (Actual y proyección): DCM
        df[1+i,2] = ifelse(i %% 2 == 1, df[i,2] , (log(df[i,3]/IDRsup)/B1)+DCMR)

        # 4° Columna Intensidad de Corta (IC %)
        df[1+i,4] = ifelse(i %% 2 == 1, (df[i,3]-df[i+1,3])/df[i,3] * 100, 0)
      ...
      for (i in 1:nrows) {
        ...
        # 3° Columna Número de árboles (Residuales): N
        df[1+i,3] = ifelse(i %% 2 == 0, IDRinf*exp(B1*(df[i,2]-DCMR)), df[i,3])

        # 2° Columna Diámetro Cuadrático Medio (Actual y proyección): DCM
        df[1+i,2] = ifelse(i %% 2 == 0, df[i,2] , (log(df[i,3]/IDRsup)/B1)+DCMR)

        # 4° Columna Intensidad de Corta (IC %)
        df[1+i,4] = ifelse(i %% 2 == 0, (df[i,3]-df[i+1,3])/df[i,3] * 100, 0)
        ...
        df[nrows+2,2] = (log(df[i+1,3]/IDRsup)/B1)+DCMR
